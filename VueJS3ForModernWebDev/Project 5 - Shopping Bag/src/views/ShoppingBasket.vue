          ...
          <div v-for="(product, index) in productsInBag" :key="index" class="item">
            ...
            <div class="{{ product.price }}">
              <span class="quantity-area">
                <button :disabled="product.quantity<=1" @click="this.$store.dispatch('decreaseQuantity', product)">-</button>
                <span class="quantity">{{ product.quantity }}</span>
                <button @click="this.$store.dispatch('increaseQuantity', product)">+</button>
              </span>
