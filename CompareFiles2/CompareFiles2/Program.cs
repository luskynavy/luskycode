...
namespace CompareFiles
{
    ...
    internal class Program
    {
        ...
        /// <returns></returns>
        private static Results FindDstNotPresentOrDifferentSize(string[] filesSrc, string[] filesDst, Options options)
        {
            Results results = new();
            ...
            while (indexSrc < filesSrc.Length && indexDst < filesDst.Length)
            {
                ...
                if (compareOrdinal == 0)
                {
                    long lengthSrc = new FileInfo(options.srcPath + "\\" + filesSrc[indexSrc]).Length;
                    long lengthDst = new FileInfo(options.dstPath + "\\" + filesDst[indexDst]).Length;

                    if (lengthSrc != lengthDst)
                    {
                        results.differentSize.Add(filesSrc[indexSrc]);
                    }
                ...
                {
                    results.onlyInDestination.Add(filesDst[indexDst]);
                    indexDst++;
                }
                //srcPath < dstPath
                ...
                {
                    results.onlyInSource.Add(filesSrc[indexSrc]);
                    indexSrc++;
                }
            }
